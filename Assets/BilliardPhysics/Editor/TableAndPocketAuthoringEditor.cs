...
namespace BilliardPhysics.Editor
{
    ...
    public class TableAndPocketAuthoringEditor : UnityEditor.Editor
    {
        ...
        // Factors used by the Scale Down ÷100 and Scale Up ×100 operations.
        private const float k_scaleDownFactor = 0.01f;
        private const float k_scaleUpFactor   = 100f;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Scale Down \u00f7100"))
            {
                ...
                if (confirmed)
                {
                    ...
                    ScaleTableConfig(auth.Table, k_scaleDownFactor);
                    if (auth.Pockets != null)
                        foreach (var pocket in auth.Pockets)
                            ScalePocketConfig(pocket, k_scaleDownFactor);
                    EditorUtility.SetDirty(target);
                    SceneView.RepaintAll();
                }
            }

            if (GUILayout.Button("Scale Up \u00d7100"))
            {
                bool confirmed = EditorUtility.DisplayDialog(
                    "Scale Up \u00d7100",
                    "This will multiply all table/pocket coordinates and pocket radii by 100.\n\nThis operation is applied in-place and can be undone.",
                    "Apply",
                    "Cancel");
                if (confirmed)
                {
                    var auth = (TableAndPocketAuthoring)target;
                    Undo.RecordObject(target, "Scale Up \u00d7100");
                    ScaleTableConfig(auth.Table, k_scaleUpFactor);
                    if (auth.Pockets != null)
                        foreach (var pocket in auth.Pockets)
                            ScalePocketConfig(pocket, k_scaleUpFactor);
                    EditorUtility.SetDirty(target);
                    SceneView.RepaintAll();
                }
            }
        ...
        // ── Scale Helpers ─────────────────────────────────────────────────
        private static void ScaleSegmentData(SegmentData seg, float factor)
        {
            if (seg == null) return;
            seg.Start = seg.Start * factor;
            seg.End   = seg.End   * factor;
            if (seg.ConnectionPoints != null)
                for (int i = 0; i < seg.ConnectionPoints.Count; i++)
                    seg.ConnectionPoints[i] = seg.ConnectionPoints[i] * factor;
        }

        private static void ScaleTableConfig(TableConfig table, float factor)
        {
            if (table?.Segments == null) return;
            foreach (var seg in table.Segments)
        ...
        private bool TryDeleteSelected()
        {
            ...
            if (_selKind == SelectionKind.TableRollPath)
            {
                ...
                if (_selPointKind == SelectionPoint.SegEnd)
                {
                    Undo.RecordObject(target, "Remove Roll Path End");
                    bool ok = RimSegmentHelper.TryPromoteLastCPToEnd(rollPath);
                    if (ok)
                    {
                        serializedObject.Update();
                        _selPointKind = SelectionPoint.None;
                ...
                if (_selPointKind == SelectionPoint.SegStart)
                {
                    Undo.RecordObject(target, "Remove Roll Path Start");
                    bool ok = RimSegmentHelper.TryPromoteFirstCPToStart(rollPath);
                    if (ok)
                    {
                        serializedObject.Update();
                        _selPointKind = SelectionPoint.None;
                ...
                // No sub-element selected: clear selection.
                ClearSelection();
                return true;
            }
